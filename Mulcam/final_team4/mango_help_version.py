...
# df = pd.read_csv('./data/api/seoul_total_id.csv') 채원 - 지워도 됨
## id
s_id = df['id']
## 식당 명
name = df['s_name']
...
for i in range(len(df)):
    ...
    try:
        ...
        if road[i] == driver.find_element(By.XPATH, '/html/body/main/article/div[1]/div[1]/div/section[1]/table/tbody/tr[1]/td').text.split(' ')[2]:
            ...
            for i in range(2, len(tr)+1):
                ...
                if what == NoSuchElementException:
                    ...
                    find_th = driver.find_element(By.XPATH, f'/html/body/main/article/div[1]/div[1]/div/section[1]/table/tbody/tr[{i}]/th')
                    find_td = driver.find_element(By.XPATH, f'/html/body/main/article/div[1]/div[1]/div/section[1]/table/tbody/tr[{i}]/td')

                    # 전화번호
                    if find_th.text == '전화번호': store['s_tel'] = find_td.text
                    # 대표메뉴    
                    if find_th.text == '음식 종류': store['s_repre'] = find_td.text
                    # 주차
                    if find_th.text == '주차': store['s_parking'] = find_td.text
                    # 영업시간
                    if find_th.text == '영업시간': store['s_hour'] = find_td.text
                    # 쉬는시간
                    if find_th.text == '쉬는시간': store['s_break'] = find_td.text
                    # 마지막주문
                    if find_th.text == '마지막주문': store['s_lastorder'] = find_td.text
                    # 휴일
                    if find_th.text == '휴일': store['s_holiday'] = find_td.text
                    # 웹 사이트
                    if find_th.text == '웹 사이트': store['s_link'] = driver.find_element(By.XPATH, f'/html/body/main/article/div[1]/div[1]/div/section[1]/table/tbody/tr[{i}]/td/a').get_attribute('href')
        ...
        count += 1
        city.append(store)
        city_nosearch.append(store)
        mango_nosearch['store'] = city_nosearch
        with open('mango_nosearch.json', 'w', encoding='utf-8') as f:
            json.dump(mango_nosearch, f, ensure_ascii=False, indent = "\t")
